...
import { Maximize2, Pause, Play, RefreshCw, Settings } from 'lucide-react'
import './App.css'
import { Simulation } from './sim/simulation'
import type { SimControls } from './sim/types'
import type { Favorite } from './types'
import { normalizeHex, parsePaletteInput, randomizePalette } from './utils/palette'
import { SettingsPanel, ContextMenuSizeSlider } from './components/SettingsPanel'
...
function App() {
  ...
  const updateControl = <K extends keyof SimControls>(key: K, value: SimControls[K]) => {
  ...
  const handleReset = () => setPalette([...defaultPalette])
  const handleRestart = () => {
  ...
  const handleSaveFavorite = () => {
    const name = favoriteName.trim() || `Palette ${new Date().toLocaleString()}`
  ...
  const handleImportFavorites = () => {
    ...
    const cleaned = parsed
      .map((entry) => {
        const name = typeof entry?.name === 'string' ? entry.name.trim() : 'Imported palette'
        const colors = Array.isArray(entry?.colors)
          ? entry.colors.map((color) => normalizeHex(String(color))).filter(Boolean)
          : []
  ...
  useEffect(() => {
    ...
    const sim = new Simulation(canvas, controls, {
      ...
      getFactionColor: (index) =>
        paletteRef.current.length ? paletteRef.current[index % paletteRef.current.length] : '#111827',
  ...
  return (
    <div className={`app${controlsCollapsed ? ' controls-collapsed' : ''}${isFullscreen ? ' fullscreen' : ''}`}>
      <aside className="controls">
        <div className="control-bar">
          ...
          <button className="icon-button" title="Restart" onClick={handleRestart}>
            <RefreshCw size={18} />
          </button>
          <button
            className="icon-button"
        ...
        <SettingsPanel
          ...
          onAddDotsForFaction={(index) => simRef.current?.addDotsForFaction(index)}
        />
      </aside>
          ...
          <ContextMenu.Trigger asChild>
            <canvas
              ...
              onMouseMove={(event) => {
                ...
                simRef.current?.handleMouseMove(event.clientX - rect.left, event.clientY - rect.top)
              }}
              ...
              onContextMenu={(event) => {
                ...
                const rect = event.currentTarget.getBoundingClientRect()
                const dot = simRef.current?.setMenuDotAt(event.clientX - rect.left, event.clientY - rect.top)
                if (dot) {
